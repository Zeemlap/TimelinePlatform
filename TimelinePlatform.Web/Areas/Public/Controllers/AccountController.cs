...
using System;
using TimelinePlatform.Web.Identity;
...
namespace TimelinePlatform.Web.Areas.Public.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        public async Task<ActionResult> SignIn(SignInViewModel model, string returnUrl)
        {
            ...
            var result = await SignInManager.PasswordSignInExtendedAsync(model.EmailAddressOrUsername, model.Password, model.RememberMe, shouldLockout: false);
            switch (result)
            {
                ...
                default:
                    ModelState.AddModelError("", StringResources.ErrorMessage_SignInFailed);
                    return View(model);
            }
        }
        ...
        [AllowAnonymous]
        public async Task<JsonResult> PasswordStrength(string password)
        {
            var passwordStrength = await AppUtilities.PasswordStrengthService.GetPasswordStrengthAsync(password, CancellationToken.None);
            return Json((int)passwordStrength);
        }
